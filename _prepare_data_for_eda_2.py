...
def main() -> None:
    """Run the data preparation pipeline end-to-end (before handling outliers)."""

    config = replace(
        DEFAULT_CONFIG,
        analysis_start_date=DEFAULT_CONFIG.holdout_split.train.start,
        analysis_end_date=DEFAULT_CONFIG.holdout_split.test.end,
        cleaned_prices_path=DEFAULT_CONFIG.cleaned_prices_path.parent / "prices_clean_eda_2.csv",
        engineered_features_path=DEFAULT_CONFIG.engineered_features_path.parent / "prices_features_eda_2.csv"
    )
