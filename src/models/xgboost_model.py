...
def tune_across_windows(
    ...
    the globally best combo by averaging val MSE across windows.

    This is what MODEL_BRIEF means by 'averaging MSE across 4 sliding
    validation folds' — the folds are the rolling windows.
    ...
    # Average MSE across windows per combo, pick lowest
    avg = (
        tuning_df
        ...
        .sort_values(["mean_val_mse", "mean_val_mae", "max_depth", "n_estimators"])
    ...
    print(f"\n  {'depth':>5}  {'n_est':>5}  {'lr':>5}  {'avg_mse':>10}  {'avg_dir_acc':>11}")
    print(f"  {'-'*47}")
    for _, row in avg.iterrows():
        ...
        print(
            ...
            f"{row['mean_val_dir_acc']:>11.3f}{marker}"
        )
    ...
    print(f"  Avg val MAE  : {float(best['mean_val_mae']):.6f}")
...
def run_rolling_windows(
    ...
    for window_dir in window_dirs:
        ...
        metrics_row = {
            ...
            "val_mse": metrics["mse"], "val_mae": metrics["mae"],
            "val_dir_acc": metrics["directional_accuracy"],
        ...
        print(
            ...
            f"MAE={metrics['mae']:.6f}  DirAcc={metrics['directional_accuracy']:.3f}  "
            f"({len(X_vl_d)} pairs)"
        )
...
def run_variant(
    ...
    print(f"\n{'='*60}")
    print(f"XGBoost {spread_type.upper()} variant")
    print(f"{'='*60}")
...
def run_both_variants(
    ...
    OLS vs Kalman comparison is on avg val MSE (within-model).
    Cross-type Sharpe comparison happens in the backtest engine.
    """
    ols_result    = run_variant("ols",    datasets_root, preds_root, min_samples, tune, target_window)
    kalman_result = run_variant("kalman", datasets_root, preds_root, min_samples, tune, target_window)

    # Summary table
    windows = sorted(set(ols_result["window_results"]) | set(kalman_result["window_results"]))
    print(f"\n{'='*60}")
    print("OLS vs Kalman — val MSE per window")
    print(f"  {'Window':<22}  {'OLS MSE':>10}  {'Kalman MSE':>12}  {'Better':>8}")
    print(f"  {'-'*57}")
    for w in windows:
        ols_mse    = ols_result["window_results"].get(w,    {}).get("metrics", {}).get("mse", float("nan"))
        kalman_mse = kalman_result["window_results"].get(w, {}).get("metrics", {}).get("mse", float("nan"))
        better     = "OLS" if ols_mse <= kalman_mse else "Kalman"
        print(f"  {w:<22}  {ols_mse:>10.6f}  {kalman_mse:>12.6f}  {better:>8}")

    for label, res in [("OLS", ols_result), ("Kalman", kalman_result)]:
        if res["holdout"]:
            m = res["holdout"]["metrics"]
            print(
                f"\n  Holdout {label}: MSE={m['mse']:.6f}  "
                f"MAE={m['mae']:.6f}  DirAcc={m['directional_accuracy']:.3f}"
            )
